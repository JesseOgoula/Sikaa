...
export default function SignupForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    else setSuccess("Inscription réussie ! Vérifiez votre email.");
    setLoading(false);
  };

  return (
    <form onSubmit={handleSubmit} className="space-y-6 w-full">
      <h2 className="text-2xl font-extrabold text-center text-orange-500 mb-2 tracking-tight">Inscription</h2>
      ...
      {success && <div className="text-green-600 text-center font-medium bg-green-50 border border-green-200 rounded p-2">{success}</div>}
      <button
        type="submit"
